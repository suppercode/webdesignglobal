...
class PollController extends BackendApplicationController
{
  ...
  public function actionView($id)
  {
    ...
    if (Yii::app()->getModule('polls')->forceVote && $model->userCanVote()) {
   ...
   */
  public function loadModel($id, $modelClass='Poll')
  {
  ...
  public function loadVote($model)
  {
    ...
    foreach ($model->votes as $vote) {
      if ($vote->user_id == $userId) {
        if (Yii::app()->getModule('polls')->ipRestrict && $isGuest && $vote->ip_address != $_SERVER['REMOTE_ADDR'])
          continue;
        else
          return $vote;
      }
