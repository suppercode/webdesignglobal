...
$total = BackendTranslatesModel::getTotalData($paramsTrans['language'],$table,'*',"true");
$page = new CPagination($total);

$page->pageSize = 20;
$curr_page = $page->getCurrentPage();
$data = BackendTranslatesModel::getListData($paramsTrans['language'],$page->getLimit(), $page->getOffset());
?>
...
<th width="50"><?php echo Yii::t('app','ID')?></th>
...
if(is_array($Ctrans) && $Ctrans["title"]!='')
	echo $Ctrans["title"];
?>
</td>
<td><?php if($value['published']!=''):?>
<a href="<?php echo Yii::app()->createUrl('translate/filterTranslate/UpdateTrans', array('id'=>$value['product_id']));?>" class="btn btn-success btn-sm" style="color: #fff"><i class="glyphicon glyphicon-ok"></i>&nbsp;<?php echo Yii::t('app','Edit');?></a>
<?php else:?>
<a href="<?php echo Yii::app()->createUrl('translate/filterTranslate/create', array('id'=>$value['product_id']))?>" class="btn btn-warning btn-sm" style="color: #fff"><?php echo Yii::t('app','Translate');?></a>
<?php endif;?>
</td>
<td width="50">
